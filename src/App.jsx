...
const useTyping = (phrases) => {
  ...
  useEffect(() => {
    const ph = phrases[pi];
    const t = setTimeout(() => {
      if (!del) { setText(ph.slice(0, ci + 1)); if (ci + 1 === ph.length) setTimeout(() => setDel(true), 1500); else setCi(c => c + 1); }
  ...
  return (
    <div style={{ background: "var(--forest)", minHeight: "100vh", overflowX: "hidden" }}>
          ...
          {feedTab === "written" && (
            <>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "1.2rem" }} className="reviews-grid">
                {WRITTEN.slice(reviewPage * 3, (reviewPage + 1) * 3).map((r, i) => (
              ...
              <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 20, marginTop: "2.5rem" }}>
                <button onClick={() => setReviewPage(p => Math.max(0, p - 1))} style={{ background: "none", border: "none", color: "var(--champagne)", fontSize: "1.5rem", cursor: "pointer", opacity: reviewPage === 0 ? 0.3 : 1, transition: "opacity 0.3s" }}>←</button>
                <div style={{ display: "flex", gap: 10 }}>
                  {[...Array(Math.ceil(WRITTEN.length / 3))].map((_, p) => (
                    <button key={p} onClick={() => setReviewPage(p)} style={{ width: 10, height: 10, borderRadius: "50%", background: reviewPage === p ? "var(--champagne)" : "rgba(212,168,83,0.2)", border: "none", cursor: "pointer", transition: "all 0.3s" }} />
                  ))}
                </div>
                <button onClick={() => setReviewPage(p => Math.min(Math.ceil(WRITTEN.length / 3) - 1, p + 1))} style={{ background: "none", border: "none", color: "var(--champagne)", fontSize: "1.5rem", cursor: "pointer", opacity: reviewPage === Math.ceil(WRITTEN.length / 3) - 1 ? 0.3 : 1, transition: "opacity 0.3s" }}>→</button>
              </div>
            </>
          )}
      ...
      <div style={{ background: "linear-gradient(135deg,var(--champagne),var(--champ2))", padding: "5rem 1.25rem", textAlign: "center", position: "relative", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div className="f-bar" style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: "100%", textAlign: "center", fontSize: "clamp(3.5rem,14vw,18rem)", fontWeight: 900, color: "rgba(15,35,24,0.06)", letterSpacing: "-2px", userSelect: "none", pointerEvents: "none", lineHeight: 1, whiteSpace: "nowrap" }}>ENROLL NOW</div>
        <Reveal dir="up" style={{ position: "relative", zIndex: 1 }}>
          ...
          <p style={{ fontSize: "1rem", color: "rgba(15,35,24,0.7)", marginBottom: "2.5rem", fontWeight: 400, maxWidth: 600, margin: "0 auto 2.5rem" }}>Transform your English today. Secure your seat now and feel the difference.</p>
          <div style={{ display: "flex", gap: "1.2rem", justifyContent: "center", flexWrap: "wrap" }}>
            ...
            <a href="tel:+919747425399" className="f-bar" style={{ padding: "1.1rem 2.2rem", borderRadius: 4, fontSize: "1.1rem", letterSpacing: "1.5px", border: "2.5px solid rgba(15,35,24,0.25)", color: "var(--forest)", textDecoration: "none", fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 8 }}>
        ...
        <div style={S.sec}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1.1fr", gap: "5rem", alignItems: "start" }} className="contact-grid">
            <Reveal dir="left">
              ...
              <p style={S.sub}>Reach out to enroll or ask questions — we'd love to hear from you.</p>
              <div style={{ marginTop: "2.5rem", display: "flex", flexDirection: "column", gap: 0 }}>
                {[["📍", "Address", "SpeakWell English Academy, Tirurangadi, Kerala, India"], ["📞", "Phone / WhatsApp", "+91 97474 25399"], ["📧", "Email", "shafi@speakwellacademy.com"], ["⏰", "Class Timings", "Morning · Evening · Weekend Batches"]].map(([ic, lbl, val]) => (
            ...
            <Reveal dir="right">
              <div style={{ background: "var(--forest2)", borderRadius: 20, padding: "2.5rem", border: "1px solid rgba(212,168,83,0.15)", position: "relative", overflow: "hidden" }}>
                ...
                <h3 className="f-play" style={{ fontSize: "1.7rem", fontWeight: 900, color: "var(--cream)", marginBottom: 6 }}>Enroll Now 🎓</h3>
                ...
                {formSent ? (
                  ...
                  <>
                    <form onSubmit={(e) => {
                      ...
                      window.open(`https://wa.me/919747425399?text=${encodeURIComponent(text)}`, "_blank");
                      setFormSent(true);
                    }}>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1rem" }}>
                        <div><label className="sw-label">Full Name</label><input required name="name" type="text" placeholder="Your name" className="sw-input" /></div>
                        <div><label className="sw-label">Phone / WhatsApp</label><input required name="phone" type="tel" placeholder="+91 XXXXX XXXXX" className="sw-input" /></div>
                      </div>
                      <div style={{ marginBottom: "1rem" }}><label className="sw-label">Email</label><input required name="email" type="email" placeholder="your@email.com" className="sw-input" /></div>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1rem" }}>
                        <div><label className="sw-label">Course</label>
                          <select required name="course" className="sw-input">
                            <option value="">Select…</option>
                            {["Spoken English Foundation", "Business English Mastery", "Public Speaking", "Interview Prep", "Young Speakers", "Weekend Bootcamp"].map(o => <option key={o} value={o}>{o}</option>)}
                          </select>
                        </div>
                        <div><label className="sw-label">Mode</label>
                          <select required name="mode" className="sw-input">
                            <option value="Online">Online</option><option value="Offline – Tirurangadi">Offline – Tirurangadi</option><option value="Both okay">Both okay</option>
                          </select>
                        </div>
                      </div>
                      <div style={{ marginBottom: "1.5rem" }}><label className="sw-label">Message (Optional)</label><textarea name="message" rows={3} placeholder="Your goals or questions for Muhammed Shafi Sir…" className="sw-input" style={{ resize: "vertical" }} /></div>
                      <button type="submit" className="btn-champ f-bar" style={{ width: "100%", padding: "1.1rem", borderRadius: 8, fontSize: "1.1rem", letterSpacing: "1.5px" }}>
                        SEND ENQUIRY & ENROLL →
                      </button>
                    </form>
                  </>
      ...
      <footer style={{ background: "#050f0a", borderTop: "1px solid rgba(212,168,83,0.1)", padding: "3rem 1.25rem" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "2rem", paddingBottom: "2rem", borderBottom: "1px solid rgba(248,243,235,0.06)" }}>
            <div>
              <div className="f-play" style={{ fontSize: "1.8rem", fontWeight: 900, color: "var(--champagne)" }}>SpeakWell English <span style={{ color: "var(--cream)" }}>Academy</span></div>
